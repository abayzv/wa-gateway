...
describe("POST /api/v1/auth/register", () => {
    it("should return 400 if email, password or name is missing", async () => {
        const response = await (0, supertest_1.default)(app_1.default).post("/api/v1/auth/register").send({
            email: "",
            password: "",
        });
        expect(response.status).toBe(400);
...
describe("POST /api/v1/auth/refreshToken", () => {
    it("should return 400 if refreshToken is missing", async () => {
        const response = await (0, supertest_1.default)(app_1.default).post("/api/v1/auth/refreshToken").send({
            refreshToken: "",
        });
        expect(response.status).toBe(400);
